...
export default class Asset {
	constructor(object) {
		...
		if (object.user_id != 1) this.user_id = object.user_id;
	...
	createImageCache(canvasZoom, previewZoom, canvasStyle) {
		...
		let shouldInvertColor = canvasStyle == "blackboard" || canvasStyle == "greenboard";
		...
		switch (this.type) {
			...
			default:
				if (canvasZoom) {
					this.cachedImg = this.color
						? ImageUtils.colorImage(
								ImageUtils.resizeImage(this.img, this.zoom * canvasZoom),
								shouldInvertColor ? invertColor(this.color) : this.color
						  )
						: ImageUtils.resizeImage(this.img, this.zoom * canvasZoom);
	...
	createTextImageCache(canvasZoom, canvasStyle) {
		...
		for (let i = 0; i < lines.length; i++) {
			let lineM = opentype.measureText(font, lines[i], fontSize);
			...
			let path = font.getPath(lines[i], x, y + i * assetM.lineHeight, fontSize);
	...
	drawPartialText(canvas, canvasZoom, progress, canvasStyle) {
		...
		for (var i = 0; i < lines.length; i++) {
			...
			if (accountedFor + text.length < Math.ceil(progress * totalLengh)) {
				...
				var lastCommand = path.commands[path.commands.length - 1],
					prevCommand = path.commands[path.commands.length - 2];
