...
export default class Scene {
  ...
  constructor(object, cache, onChangeCallback) {
    this.id = object.id;
    ...
    this.items = object.items || (object.data && object.data.items ? object.data.items.map(item => new Asset(item)) : []);
    ...
    $.get(this.path + this.id)
      .done(scene => {
        ...
        if (this.items.length > 0) {
          this.items = this.items.map(item => new Asset(item));
  ...
  addAsset(item, x, y, canvasSize, callback) {
    ...
    if (item.type == 'template') {
      ...
      if (this.items.length == 0) {
        ...
        this.items = data.items.map(asset => new Asset(asset));
  ...
  draw(canvas, canvasZoom, hoverTarget, isPreview, canvasStyle, final = false, applyEffects = true) {
    ...
    if (applyEffects) {
      if (canvasStyle == 'blackboard' || canvasStyle == 'greenboard')
  ...
  drawExitAnimation(canvas, canvasZoom, progress, isPreview, canvasStyle) {
    ...
    let alpha = (canvasStyle == 'blackboard' || canvasStyle == 'greenboard') ? 0.04 : 0.06;
  ...
  getThumbnail() {
    if (this.prev_thumbnail_data != this.currentData) {
      ...
      let canvasStyle = (this.cachedCanvasStyle == 'greenboard') ? 'greenboard' : 'whiteboard';
