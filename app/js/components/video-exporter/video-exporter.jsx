...
class VideoExporter extends React.Component {
  ...
  changeResolution = (e) => {
    ...
    this.setState({
      width: 2 * Math.round(height * 1.77778 / 2), // always divisible by 2
      height: height,
      zoom: height / 648,
  ...
  export = () => {
    ...
    var scaleRatio = this.state.height / 648; // editor height
    ...
    if (Math.round(this.state.elapsed) % Math.round(2000) < Math.floor(1000 / this.state.fps) && this.props.video.style == 'blackboard')
  ...
  renderFrame = () => {
    ...
    this.props.video.draw(canvas, this.state.zoom, elapsed, false, this.state.fps);
  ...
  exportStillFrames = () => {
    ...
    this.props.video.draw(canvas, this.state.zoom, this.state.elapsed + 1000 / this.state.fps, false);
...
function getBatchSize(fps, resolutionHeight) {
  return Math.round(BATCH_SIZE * 30 / fps * (648 / resolutionHeight));
}
