...
export default class Video {
  ...
  constructor(item, onLoadCallback) {
    ...
    Hand.useSet(this.handSet, this.style);
    ...
    $.get(this.path)
      .done(video => {
        this.scenes = video.scenes.map((scene, i) => {
          scene.order = i + 1;
          return new Scene(scene, this.cache);
        });
        callback();
      })
  ...
  draw(canvas, canvasZoom, elapsed, isPreview, fps = 15) {
    var time = Date.now();
    ...
    if (sceneInfo) {
      if (sceneInfo.erasing) {
        Hand.drawEraser(handLayer, sceneInfo.handInfo.x, sceneInfo.handInfo.y, sceneInfo.useFinger);
      }
      else
        this.prevHand = Hand.draw(handLayer, sceneInfo.handInfo.x, sceneInfo.handInfo.y, sceneInfo.handInfo.angle, this.prevHand, fps);
    }

    var ctx = canvas.getContext('2d');
    ctx.globalCompositeOperation = 'source-over';
    if (this.style == 'blackboard') {
      ctx.drawImage(chalkboard, 0, 0, canvas.width, canvas.height);
    } else if (this.style == 'greenboard'){
    ...
    if (this.style == 'glassboard')
      ctx.drawImage(handLayer, 0, 0);

    if (this.style == 'glassboard')
      ctx.globalCompositeOperation = 'multiply';
    if (this.style == 'blackboard' || this.style == 'greenboard')
      ctx.globalCompositeOperation = 'screen';

    ctx.drawImage(background, (canvas.width - frame.width) / 2, (canvas.height - frame.height) / 2, frame.width, frame.height);
    ctx.drawImage(frame, (canvas.width - frame.width) / 2, (canvas.height - frame.height) / 2, frame.width, frame.height);

    ctx.globalCompositeOperation = 'source-over';

    if (['whiteboard', 'blackboard', 'greenboard'].includes(this.style))
      ctx.drawImage(handLayer, 0, 0);
